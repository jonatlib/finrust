...
pub struct ScenarioUnreconciled {}

impl ScenarioUnreconciled {
    pub fn new() -> Self {
        Self {}
    }
}
...
#[async_trait]
impl TestScenarioBuilder for ScenarioUnreconciled {
    async fn get_scenario(&self) -> Result<TestScenario, DbErr> {
        ...
        let account = account::ActiveModel {
            ...
            description: Set(Some("Account for unreconciled testing".to_string())),
        ...
        let unreconciled_tx = recurring_transaction::ActiveModel {
        ...
        // Create assertions for the balance model
        // For the balance model, unreconciled recurring transactions are ignored
        // January 31: Initial $1000 (manual state takes precedence over the reconciled rent)
        // February 28: $1000 - $500 (reconciled rent) - $150 (groceries) = $350
        // March 31: $350 (no more reconciled transactions) = $350
        let balance_assert_results: AssertResult = vec![
            (
                account.id,
        ...
        // For the forecast model, past unreconciled transactions are moved forward to today
        // Assuming today is 2022-12-31 (before all test dates)
        // January 1: Initial $1000 - $500 (reconciled rent) = $500
        // January 31: $500 - $100 (unreconciled subscription on Jan 1, moved to Dec 31) = $400
        // February 1: $400 - $500 (reconciled rent) = -$100
        // February 28: -$100 - $150 (groceries) = -$250
        // March 1: -$250 - $100 (unreconciled subscription on Mar 1) = -$350
        // March 31: -$350 - $500 (unreconciled rent on Mar 1) = -$850
        let _forecast_assert_results: AssertResult = vec![
            (
                account.id,
                NaiveDate::from_ymd_opt(2023, 1, 31).unwrap(),
            ...
            (
                account.id,
                NaiveDate::from_ymd_opt(2023, 3, 31).unwrap(),
                Decimal::new(-85000, 2), // -$850.00
            ),
        ];

        // Return the test scenario with balance assertions
        // The test_scenario_unreconciled_balance test will use these assertions
        Ok((db, vec![account], balance_assert_results))
    }
}
