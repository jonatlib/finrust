...
pub fn compute_stats_vec_to_collection(
    stats_data: Vec<(i32, Option<Decimal>, Option<Decimal>, Option<Decimal>, Option<Decimal>, Option<Decimal>, Option<Decimal>)>,
    period: TimePeriod,
) -> AccountStatisticsCollection {
    let statistics: Vec<AccountStatistics> = stats_data
        .into_iter()
        .map(|(account_id, min_state, max_state, average_expense, average_income, upcoming_expenses, end_of_period_state)| {
...
) -> Vec<(i32, Option<String>, Option<String>, Option<String>, Option<String>, Option<String>, Option<String>)> {
    collection
        .statistics
        .iter()
        .map(|stats| (
...
mod tests {
    ...
    fn test_create_account_state_point() {
        let point = create_account_state_point(1, NaiveDate::from_ymd_opt(2024, 1, 1).unwrap(), "100.50")
    ...
    fn test_create_account_state_point_invalid_balance() {
        let result = create_account_state_point(1, NaiveDate::from_ymd_opt(2024, 1, 1).unwrap(), "invalid");
    ...
    fn test_create_account_state_points() {
        let data = vec![
            (1, NaiveDate::from_ymd_opt(2024, 1, 1).unwrap(), "100.00".to_string()),
            (2, NaiveDate::from_ymd_opt(2024, 1, 1).unwrap(), "200.00".to_string()),
        ];
    ...
    fn test_compute_stats_vec_to_collection() {
        let stats_data = vec![
            (1, Some(Decimal::new(100, 2)), Some(Decimal::new(500, 2)), None, None, None, None),
            (2, Some(Decimal::new(200, 2)), Some(Decimal::new(600, 2)), None, None, None, None),
        ];
    ...
    fn test_period_creation_functions() {
        ...
        assert_eq!(create_date_range_period(start, end), TimePeriod::date_range(start, end));
    }
}